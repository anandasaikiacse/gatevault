...
function getWardenStatus(pass: WardenPass): "Pending" | "Approved" | "Rejected" {
  return pass.wardenApprovalStatus || pass.approvalStatus || "Pending";
}

function getStudentName(pass: WardenPass) {
...
export default function WardenDashboard() {
  ...
  const [passes, setPasses] = useState<WardenPass[]>([]);
  const [logs, setLogs] = useState<WardenPass[]>([]);
  ...
  const [selectedStudent, setSelectedStudent] = useState<QueueRow | null>(null);
  const [calendarDate, setCalendarDate] = useState(() => new Date());
  const [darkMode, setDarkMode] = useState(false);
  const [activeView, setActiveView] = useState<DashboardView>("dashboard");

  const isWarden = (session?.user as { role?: string } | undefined)?.role === "warden";
  ...
  const fetchPasses = useCallback(async (showLoading = false) => {
    ...
    try {
      const res = await fetch(`/api/warden/passes?ts=${Date.now()}`, {
        cache: "no-store",
      });
      const data = await res.json();

      if (!res.ok) {
        throw new Error(data.message || "Failed to load long leave requests");
      }
  ...
  useEffect(() => {
    ...
    if (status === "authenticated") {
      if (!isWarden) {
        setError("Warden access required");
        setLoading(false);
        return;
      }
  ...
  }, [status, isWarden, router, fetchPasses]);

  useEffect(() => {
    if (status !== "authenticated" || !isWarden) {
      return;
    }
  ...
  const decidePass = async (passId: string, action: "approve" | "reject") => {
    ...
    try {
      const res = await fetch("/api/warden/passes", {
        method: "PATCH",
      ...
      if (!res.ok) {
        throw new Error(data.message || "Failed to update long leave request");
      }
  ...
  const rows = useMemo<QueueRow[]>(() => {
    ...
    const logRows = logs.map((log) => ({ ...log, queueStatus: getWardenStatus(log), source: "Log" as const }));
  ...
  const filteredRows = useMemo(() => {
    ...
    const filtered = rows.filter((row) => {
      ...
      return (!loweredSearch || haystack.includes(loweredSearch)) && (statusFilter === "All" || row.queueStatus === statusFilter);
    });
  ...
  const analytics = useMemo(() => {
    ...
    return { approved, rejected, pending, total, approvalRate };
  }, [passes.length, rows]);
  ...
  const calendarRequests = useMemo(() => {
    ...
    rows.forEach((row) => {
      const key = safeDate(row.leaveStartDate || row.createdAt).toISOString().slice(0, 10);
  ...
  const exportRows = (type: "csv" | "excel") => {
    const header = ["Student", "Email", "Phone", "Place", "Purpose", "Leave Start", "Leave End", "Time Out", "Time In", "Status", "Updated"];
    ...
    downloadBlob(`warden-long-leave-${new Date().toISOString().slice(0, 10)}.${extension}`, content, mime);
  };
        ...
        <aside className="rounded-[28px] bg-[#111111] p-5 text-white shadow-2xl lg:min-h-[760px]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/46">Warden Console</p>
            </div>
          </div>
          ...
          <div className="mt-10 rounded-3xl bg-white/8 p-4">
            ...
            <p className="mt-2 text-xs leading-5 text-white/56">Requests refresh automatically while this page is open. QR unlocks after your approval.</p>
        ...
        <section className="min-w-0 space-y-4">
          <header className={`rounded-[28px] border p-5 shadow-xl backdrop-blur-2xl ${panelClass}`}>
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className={`text-xs font-semibold uppercase tracking-wide ${mutedText}`}>Long leave final approvals</p>
                <h1 className="mt-2 text-3xl font-semibold tracking-tight">Good morning, Warden</h1>
                <p className={`mt-2 max-w-2xl text-sm leading-6 ${mutedText}`}>
                  Review HOD-approved long leave requests from your assigned hostel, inspect student details, export logs, and release QR access after final approval.
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button type="button" onClick={() => router.push("/warden/profile")} className={`inline-flex h-11 items-center gap-2 rounded-2xl border px-4 text-sm font-semibold ${panelClass}`}>
                  <User size={16} />
                  Profile
                </button>
              </div>
            ...
            <article className={`rounded-[28px] border p-5 shadow-xl backdrop-blur-2xl ${panelClass}`}>
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className={`text-sm ${mutedText}`}>Last seven days by status</p>
                </div>
              ...
              <div className="mt-4 h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={weeklyChart}>
                    <defs>
                      <linearGradient id="approvedGradient" x1="0" x2="0" y1="0" y2="1">
                    ...
                    <Tooltip />
                    <Area type="monotone" dataKey="Approved" stroke="#10b981" fill="url(#approvedGradient)" strokeWidth={3} />
          ...
          <section className="grid gap-4 xl:grid-cols-1">
            ...
            <article className={`rounded-[28px] border p-5 shadow-xl backdrop-blur-2xl ${panelClass}`}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h2 className="text-lg font-semibold">HOD Approved Queue</h2>
                  <p className={`text-sm ${mutedText}`}>Search, filter, sort, inspect, export, approve, or reject final hostel requests</p>
                </div>
                <div className="flex flex-wrap gap-2">
                    ...
                    {filteredRows.map((row) => (
                        <tr key={`${row.source}-${row._id}`} className={`border-t ${darkMode ? "border-white/8" : "border-slate-200/70"}`}>
                          <td className="px-4 py-3">
                            <button type="button" onClick={() => setSelectedStudent(row)} className="text-left">
                          ...
                          <td className="px-4 py-3">
                            ...
                            <p className={`text-xs ${mutedText}`}>{formatDate(row.leaveStartDate)} - {formatDate(row.leaveEndDate)} - {row.timeOut} to {row.timeIn}</p>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            {row.queueStatus === "Pending" ? (
                              ...
                              <button type="button" onClick={() => setSelectedStudent(row)} className="rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold">
                                View
                              </button>
                            )}
                          </td>
            ...
            <article className={`rounded-[28px] border p-5 shadow-xl backdrop-blur-2xl ${panelClass}`}>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {rows.slice(0, 6).map((row) => (
                  <button key={`calendar-timeline-${row.source}-${row._id}`} type="button" onClick={() => setSelectedStudent(row)} className={`rounded-2xl border p-3 text-left ${solidPanelClass}`}>
          ...
          <section className="grid gap-4 xl:grid-cols-3">
            <article className={`rounded-[28px] border p-5 shadow-xl backdrop-blur-2xl ${panelClass}`}>
              ...
              <p className={`mt-2 text-sm ${mutedText}`}>Pull the latest HOD-approved hostel requests and logs from the server.</p>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/58 p-4 backdrop-blur-sm">
          <section className={`max-h-[90vh] w-full max-w-2xl overflow-auto rounded-[28px] border p-6 shadow-2xl ${darkMode ? "border-white/10 bg-slate-950 text-white" : "border-white/70 bg-white text-slate-900"}`}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className={`text-xs font-semibold uppercase tracking-wide ${mutedText}`}>Student profile</p>
                <h2 className="mt-1 text-2xl font-semibold">{getStudentName(selectedStudent)}</h2>
                <p className={`mt-1 text-sm ${mutedText}`}>{selectedStudent.user?.email || selectedStudent.phone}</p>
              </div>
              <button type="button" onClick={() => setSelectedStudent(null)} className="grid h-10 w-10 place-items-center rounded-2xl bg-slate-950 text-white">
                <X size={17} />
              </button>
            </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              {[
                ...
                { label: "Room", value: `${selectedStudent.user?.hostel || "-"} / ${selectedStudent.user?.room || "-"}` },
                { label: "Status", value: selectedStudent.queueStatus },
              ].map((item) => (
            ...
            <div className="mt-5 rounded-2xl border border-slate-200/60 p-4">
              ...
              <p className={`mt-2 text-sm leading-6 ${mutedText}`}>
                {selectedStudent.purpose} at {selectedStudent.place}. Leave window {formatDate(selectedStudent.leaveStartDate)} to {formatDate(selectedStudent.leaveEndDate)}, {selectedStudent.timeOut} to {selectedStudent.timeIn}.
              </p>
            </div>
              ...
              <div className="mt-3 space-y-2">
                {selectedHistory.map((item) => (
                  <div key={`history-${item.source}-${item._id}`} className={darkMode ? "rounded-2xl bg-white/10 p-3" : "rounded-2xl bg-slate-100 p-3"}>
                    ...
                    <p className={`mt-1 text-xs ${mutedText}`}>{formatDate(item.leaveStartDate)} - {item.purpose}</p>
                  </div>
                ))}
              </div>
            </div>
