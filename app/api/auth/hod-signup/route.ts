...
export async function POST(req: Request) {
  try {
    ...
    const limit = rateLimit(`hod-signup:${getClientIp(req)}`, 5, 60000);
    if (!limit.allowed) {
    ...
    const existingHod = await User.exists({ role: "hod" });
    ...
    if (existingHod) {
      ...
      if (!currentUser || !["admin", "hod"].includes(currentUser.role)) {
        return NextResponse.json(
          { message: "Only an admin or existing HOD can create another HOD account" },
          { status: 403 }
        );
      }
    }
    ...
    return NextResponse.json({ message: "HOD account created successfully" }, { status: 201 });
  } catch (error: unknown) {
