...
function getStatus(pass: HodPass): "Pending" | "Approved" | "Rejected" {
  return pass.hodApprovalStatus || pass.approvalStatus || "Pending";
}

function getStudentName(pass: HodPass) {
...
export default function HodDashboard() {
  ...
  const [passes, setPasses] = useState<HodPass[]>([]);
  const [logs, setLogs] = useState<HodPass[]>([]);
  ...
  const rows = useMemo<QueueRow[]>(() => {
    ...
    const logRows = logs.map((log) => ({ ...log, queueStatus: getStatus(log), source: "Log" as const }));
  ...
  const exportRows = (type: "csv" | "excel") => {
    ...
    downloadBlob(`hod-long-leave-${new Date().toISOString().slice(0, 10)}.${extension}`, content, mime);
  };
        ...
        <aside className="rounded-[28px] bg-[#111111] p-5 text-white shadow-2xl lg:min-h-[760px]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/46">HOD Console</p>
            </div>
          </div>
        ...
        <section className="min-w-0 space-y-4">
          <header className={`rounded-[28px] border p-5 shadow-xl backdrop-blur-2xl ${panelClass}`}>
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className={`text-xs font-semibold uppercase tracking-wide ${mutedText}`}>Long leave approvals</p>
                <h1 className="mt-2 text-3xl font-semibold tracking-tight">Good morning, HOD</h1>
                <p className={`mt-2 max-w-2xl text-sm leading-6 ${mutedText}`}>
                  Review long leave requests, inspect student details, export logs, and plan upcoming approvals from one desk.
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button type="button" onClick={() => router.push("/hod/profile")} className={`inline-flex h-11 items-center gap-2 rounded-2xl border px-4 text-sm font-semibold ${panelClass}`}>
                  <User size={16} />
                  Profile
                </button>
              </div>
